...
load(fullfile('data-fitted', 'Step2A_AmpEstd.mat'));
...
load(fullfile('data-fitted', 'Step2B_SpecAmp.mat'));
...
hold on;
rectangle('Position', [wFFT(1), -2.5, diff(wFFT), 5], ...
	'EdgeColor', 'none', 'FaceColor', shadeAreaColor);  % FFT window
...
% Panel E: plot spectral amplitude distribution.
hold on;
rectangle('Position', ...
	[freqBounds(rcdfIdx, 1), -.5, diff(freqBounds(rcdfIdx, :)), 1.5], ...
	'EdgeColor', 'none', 'FaceColor', shadeBarColor);  % shaded bar
...
% Panel F: plot spectral amplitude distributions for strong/weak rhythms.
hold on;
for r = 1:2
