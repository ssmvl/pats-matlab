...
load(fullfile('data-fitted', 'Step5B_RuzzoliEtAl2019.mat'));
...
if (BF10 < 10000) && (BF10 > 0.01)
	fprintf('EstdAmp: BF\x2081\x2080=%.2f, p=%.3f\n', BF10, pValue);
else
	fprintf('EstdAmp: BF\x2081\x2080=%.2e, p=%.3f\n', BF10, pValue);
end



% Load generated data (Example 3, second part).
load(fullfile('data-fitted', 'Step5C_RuzzoliEtAl2019.mat'));
...
specAmpExtAlpha = cellfun(@(amp) mean(amp(lbFreqIdx:ubFreqIdx)), specAmp');
[BF10, pValue]  = bf.ttest(specAmpExtAlpha(:, 1), specAmpExtAlpha(:, 2));
if (BF10 < 10000) && (BF10 > 0.01)
	fprintf('SpecAmp:  BF\x2081\x2080=%.2f, p=%.3f\n\n', BF10, pValue);
else
	fprintf('SpecAmp:  BF\x2081\x2080=%.2e, p=%.3f\n\n', BF10, pValue);
end
