...
% Panel F: plot spectral amplitude distributions for hit/miss trials.
hold on;
for r = 1:2
	avgSpecAmp = mean(cell2mat(specAmp(r, :)'), 1);
	semSpecAmp = std(cell2mat(specAmp(r, :)'), [], 1) / sqrt(fileCount);

	errorHandle = errorbar(fftFreqs, avgSpecAmp, semSpecAmp, ...
		errorBarParams{:});
	skipLegend(errorHandle);
	plot(fftFreqs, avgSpecAmp, 'Color', lineColors2(r, :), ...
		'LineWidth', plotLineWidth);
end
hold off;
...
setAxes(4:4:20, '%d\n', 0:.1:.3, '%.1f\n');
xlabel('Frequency (Hz)', labelParams{:});
ylabel('Spectral amplitude', labelParams{:});
